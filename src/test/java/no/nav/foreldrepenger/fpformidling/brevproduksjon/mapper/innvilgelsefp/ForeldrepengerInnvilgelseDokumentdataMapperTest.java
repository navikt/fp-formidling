...
import static no.nav.foreldrepenger.fpformidling.integrasjon.fpsak.BrevGrunnlagBuilders.rettigheter;
...
class ForeldrepengerInnvilgelseDokumentdataMapperTest {
    ...
    void SjekkAtTotilkjentPerioderMedEnUttaksperiodeFårRiktigTapteDager() {
        ...
        var behandling = opprettBehandling(BehandlingType.REVURDERING,
            ...
            .beregningsgrunnlag(opprettBeregningsgrunnlag(AktivitetStatusDto.ARBEIDSTAKER))
            .foreldrepengerUttak(opprettUttaksresultat2())
            .build();
        var dokumentFelles = lagStandardDokumentFelles(FagsakYtelseType.FORELDREPENGER);
        var dokumentHendelse = lagStandardHendelseBuilder().build();
    ...
    private BrevGrunnlagBuilder opprettBehandling(BehandlingType behandlingType,
        ...
        return brevGrunnlag().saksnummer(SAKSNUMMER)
            ...
            .rettigheter(rettigheter().opprinnelig(Rettigheter.Rettighetstype.BEGGE_RETT).gjeldende(Rettigheter.Rettighetstype.BEGGE_RETT).build())
    ...
    private BeregningsgrunnlagDto opprettBeregningsgrunnlag(AktivitetStatusDto aktivitetStatus) {
        return new BeregningsgrunnlagDto(List.of(aktivitetStatus), null, BigDecimal.valueOf(GRUNNBELØP),
    ...
    private ForeldrepengerUttak opprettUttaksresultat() {
        var uttakAktivitet = foreldrepengerUttakAktivitet().trekkdager(BigDecimal.TEN)
            ...
            .trekkontoType(ForeldrepengerUttak.TrekkontoType.FORELDREPENGER)
            .build();
        var uttakAktivitet1 = foreldrepengerUttakAktivitet().trekkdager(BigDecimal.TEN)
            .utbetalingsgrad(BigDecimal.ZERO)
            .uttakArbeidType(UttakArbeidType.ORDINÆRT_ARBEID)
            .prosentArbeid(BigDecimal.valueOf(200))
            .trekkontoType(ForeldrepengerUttak.TrekkontoType.FORELDREPENGER_FØR_FØDSEL)
            .build();
        ...
        return foreldrepengerUttak().stønadskontoer(List.of(stønadskonto().stønadskontotype(ForeldrepengerUttak.Stønadskonto.Type.MØDREKVOTE)
                ...
                .build(), stønadskonto().stønadskontotype(ForeldrepengerUttak.Stønadskonto.Type.FELLESPERIODE)
                .maxDager(5)
    ...
    private long finnDagsats(TilkjentYtelseDagytelseDto tilkjentYtelse) {
        return tilkjentYtelse.perioder().stream().mapToLong(TilkjentYtelseDagytelseDto.TilkjentYtelsePeriodeDto::dagsats).max().orElse(0);
    }

    private long finnMånedsbeløp(TilkjentYtelseDagytelseDto tilkjentYtelse) {
        return finnDagsats(tilkjentYtelse) * 260 / 12;
    }
}
