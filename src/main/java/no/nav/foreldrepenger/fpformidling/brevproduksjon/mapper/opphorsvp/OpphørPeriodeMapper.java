...
import static no.nav.foreldrepenger.fpformidling.integrasjon.fpsak.dto.behandling.BrevGrunnlag.Inntektsmelding;
...
public final class OpphørPeriodeMapper {
    ...
    public static Tuple<OpphørPeriode, String> mapOpphørtePerioderOgLovhjemmel(BrevGrunnlag behandling,
                                                                               List<SvangerskapspengerUttak.UttakArbeidsforhold> uttakResultatArbeidsforhold,
                                                                               Språkkode språkKode,
    ...
    private static OpphørPeriode mapOpphørteArbeidsforhold(List<SvangerskapspengerUttak.UttakArbeidsforhold> uttakResultatArbeidsforhold,
                                                           List<Inntektsmelding> inntektsmeldinger,
                                                           UnaryOperator<String> hentNavn) {
    ...
    private static OpphørPeriode mapOpphørtPeriodeMedÅrsakFraAvslåttUttak(List<SvangerskapspengerUttak.UttakArbeidsforhold> uttakResultatArbeidsforhold,
                                                                          Språkkode språkKode,
                                                                          List<TilkjentYtelsePeriodeDto> tilkjentYtelsePerioder,
                                                                          List<Inntektsmelding> inntektsmeldinger,
    ...
    private static PeriodeIkkeOppfyltÅrsak finnNyestePeriodeIkkeOppfyltÅrsak(List<SvangerskapspengerUttak.Periode> opphørtePerioder) {
        return opphørtePerioder.stream()
            .max(Comparator.comparing(SvangerskapspengerUttak.Periode::tom))
    ...
    private static Optional<LocalDate> finnførsteUttaksDatoFraInnvilget(List<SvangerskapspengerUttak.UttakArbeidsforhold> uttakResultatArbeidsforhold) {
        return uttakResultatArbeidsforhold.stream()
            .flatMap(ura -> ura.perioder().stream())
    ...
    private static Optional<LocalDate> finnSisteUttaksDatoFraInnvilget(List<SvangerskapspengerUttak.UttakArbeidsforhold> uttakResultatArbeidsforhold) {
        return uttakResultatArbeidsforhold.stream()
            .flatMap(ura -> ura.perioder().stream())
            .filter(ur -> PeriodeResultatType.INNVILGET.equals(ur.periodeResultatType()))
            .map(SvangerskapspengerUttak.Periode::tom)
