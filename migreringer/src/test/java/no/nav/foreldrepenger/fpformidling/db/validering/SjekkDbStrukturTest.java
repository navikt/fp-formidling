...
public class SjekkDbStrukturTest {
    ...
    void sjekk_at_alle_FK_kolonner_har_fornuftig_indekser() throws Exception {
        var sql = "SELECT " + "  uc.table_name, uc.constraint_name, LISTAGG(dcc.column_name, ',') WITHIN GROUP (ORDER BY dcc.position) as columns"
            + " FROM all_Constraints Uc" + "   INNER JOIN all_cons_columns dcc ON dcc.constraint_name  =uc.constraint_name AND dcc.owner=uc.owner"
            + " WHERE Uc.Constraint_Type='R'" + "   AND Uc.Owner            = upper(?)" + "   AND Dcc.Column_Name NOT LIKE 'KL_%'" + "   AND EXISTS"
            + "       (SELECT ucc.position, ucc.column_name" + "         FROM all_cons_columns ucc"
            + "         WHERE Ucc.Constraint_Name=Uc.Constraint_Name" + "           AND Uc.Owner             =Ucc.Owner"
            + "           AND ucc.column_name NOT LIKE 'KL_%'" + "       MINUS" + "        SELECT uic.column_position AS position, uic.column_name"
            + "        FROM all_ind_columns uic" + "        WHERE uic.table_name=uc.table_name" + "          AND uic.table_owner =uc.owner"
            + "       )" + " GROUP BY Uc.Table_Name, Uc.Constraint_Name" + " ORDER BY uc.table_name";
