...
import no.nav.foreldrepenger.fpformidling.datamapper.util.BrevMapperUtil;
...
public class KlageHjemsendtDokumentdataMapperTest {

    private static final LocalDate SVARFRIST = LocalDate.now().plusDays(10);
    ...
    private KlageHjemsendtDokumentdataMapper dokumentdataMapper;

    @BeforeEach
    public void before() {
        dokumentData = lagStandardDokumentData(DokumentMalType.KLAGE_HJEMSENDT);
        when(brevMapperUtil.getSvarFrist()).thenReturn(SVARFRIST);
        dokumentdataMapper = new KlageHjemsendtDokumentdataMapper(brevMapperUtil, domeneobjektProvider);
    }
    ...
    public void skal_mappe_felter_for_brevet() {
        ...
        // Act
        KlageHjemsendtDokumentdata dokumentdata = dokumentdataMapper.mapTilDokumentdata(dokumentFelles, dokumentHendelse, behandling, false);
        ...
        assertThat(dokumentdata.getOpphevet()).isTrue();
        assertThat(dokumentdata.getEttersendelsesfrist()).isEqualTo(formaterDatoNorsk(SVARFRIST));
    }
