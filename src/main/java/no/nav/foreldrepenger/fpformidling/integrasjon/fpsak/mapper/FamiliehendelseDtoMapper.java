...
public final class FamiliehendelseDtoMapper {
    ...
    private static Optional<LocalDate> finnFødselsdatoFraDto(FamiliehendelseDto dto) {
        if (dto instanceof AvklartDataFodselDto d && d.getAvklartBarn() != null) {
            return d.getAvklartBarn().stream().map(AvklartBarnDto::getFodselsdato).filter(Objects::nonNull).min(LocalDate::compareTo);
        }
    ...
    private static Optional<LocalDate> finnDødsdatoFraDto(FamiliehendelseDto dto) {
        if (dto instanceof AvklartDataFodselDto d && d.getAvklartBarn() != null) {
            return d.getAvklartBarn().stream().map(AvklartBarnDto::getDodsdato).filter(Objects::nonNull).min(LocalDate::compareTo);
        }
        return Optional.empty();
    }
    ...
    private static boolean alleFelterAdopsjonErNull(AvklartDataAdopsjonDto dto) {
        return dto.getEktefellesBarn() == null && dto.getMannAdoptererAlene() == null && dto.getAdopsjonFodelsedatoer() == null
            && dto.getAnkomstNorge() == null && dto.getOmsorgsovertakelseDato() == null;
    }

    private static boolean alleFelterFødselErNull(AvklartDataFodselDto dto) {
        return dto.getAvklartBarn() == null && dto.getBrukAntallBarnFraTps() == null && dto.getErOverstyrt() == null
            && dto.getMorForSykVedFodsel() == null && dto.getAntallBarnTermin() == null && dto.getTermindato() == null && dto.getUtstedtdato() == null
            && dto.getVedtaksDatoSomSvangerskapsuke() == null && dto.getSkjæringstidspunkt() == null;
    }

    private static boolean alleFelterOmsorgErNull(AvklartDataOmsorgDto dto) {
        return dto.getAntallBarnTilBeregning() == null && dto.getForeldreansvarDato() == null && dto.getOmsorgsovertakelseDato() == null
