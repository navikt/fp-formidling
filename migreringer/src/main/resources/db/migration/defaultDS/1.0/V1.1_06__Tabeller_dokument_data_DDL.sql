...
(
  ...
  BESTILLING_TYPE   VARCHAR(100)                        NOT NULL,
  BESTILT_TID       TIMESTAMP(3)                        NOT NULL,
  ...
  CONSTRAINT FK_DOKUMENT_DATA_3 FOREIGN KEY (DOKUMENT_MAL_NAVN) REFERENCES DOKUMENT_MAL_TYPE (KODE)
);
...
CREATE INDEX IDX_DOKUMENT_DATA_1 ON DOKUMENT_DATA (BEHANDLING_UUID);
CREATE INDEX IDX_DOKUMENT_DATA_2 ON DOKUMENT_DATA (DOKUMENT_MAL_NAVN);

COMMENT ON COLUMN DOKUMENT_DATA.ID IS 'Primary Key';
COMMENT ON COLUMN DOKUMENT_DATA.BEHANDLING_UUID IS 'BEHANDLING Fremmednøkkel for kobling til behandling';
COMMENT ON COLUMN DOKUMENT_DATA.DOKUMENT_MAL_NAVN IS 'Navn på mal som brevet er lages fra.';
COMMENT ON COLUMN DOKUMENT_DATA.BESTILLING_TYPE IS 'Dokument er brev utkast eller ikke redigerbart dokument';
...
(
  ...
  CONSTRAINT FK_DOKUMENT_FELLES_3 FOREIGN KEY (DOKUMENT_DATA_ID) REFERENCES DOKUMENT_DATA (ID),
  CONSTRAINT FK_DOKUMENT_FELLES_4 FOREIGN KEY (SPRAK_KODE, KL_SPRAAK_KODE) REFERENCES KODELISTE (KODE, KODEVERK)
);
...
CREATE INDEX IDX_DOKUMENT_FELLES_1 ON DOKUMENT_FELLES (DOKUMENT_DATA_ID);
CREATE INDEX IDX_DOKUMENT_FELLES_2 ON DOKUMENT_FELLES (SPRAK_KODE);
...
COMMENT ON COLUMN DOKUMENT_FELLES.XML IS 'XML som er sendt til dokprod';
