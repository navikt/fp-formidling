...
import no.nav.foreldrepenger.fpformidling.integrasjon.fpsak.BrevGrunnlag.ForeldrepengerUttak.Aktivitet;
...
public final class AvslåttPeriodeMapper {
    ...
    public static Tuple<List<AvslåttPeriode>, String> mapAvslåttePerioderOgLovhjemmel(BrevGrunnlag behandling,
                                                                                      List<TilkjentYtelsePeriodeDto> tilkjentYtelsePerioder,
                                                                                      Optional<BrevGrunnlag.ForeldrepengerUttak> uttakResultatPerioder,
                                                                                      Språkkode språkkode) {
    ...
    private static List<AvslåttPeriode> finnAvslåttePerioder(Collection<BrevGrunnlag.Behandlingsresultat.VilkårType> vilkårTyper,
        ...
        for (var periode : uttakResultatPerioder.map(BrevGrunnlag.ForeldrepengerUttak::perioderSøker).orElse(Collections.emptyList())) {
            ...
            if (BrevGrunnlag.PeriodeResultatType.AVSLÅTT.equals(periode.periodeResultatType()) && periodeResultatÅrsak != null) {
    ...
    private static AvslåttPeriode mapAvslåttPeriode(TilkjentYtelsePeriodeDto tilkjentYtelsePeriode,
                                                    ...
                                                    BrevGrunnlag.ForeldrepengerUttak.Periode uttakResultatPeriode,
                                                    Språkkode språkkode,
                                                    TreeSet<String> lovReferanser) {
    ...
    private static BrevGrunnlag.ForeldrepengerUttak.Periode finnUttakResultatPeriode(Optional<BrevGrunnlag.ForeldrepengerUttak> uttakResultatPerioder,
                                                                                     TilkjentYtelsePeriodeDto tilkjentYtelsePeriode) {
        return PeriodeBeregner.finnUttaksperiode(tilkjentYtelsePeriode,
            uttakResultatPerioder.map(BrevGrunnlag.ForeldrepengerUttak::perioderSøker).orElse(Collections.emptyList()));
    }
}
