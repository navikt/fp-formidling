...
class OpphørPeriodeMapperTest {
    ...
    void opphør_uten_uttak_og_tilkjent_ytelse() {
        ...
        var behandling = brevGrunnlag().uuid(UUID.randomUUID())
            .behandlingsresultat(behandlingsresultat().avslagsårsak(Avslagsårsak.SØKER_SKULLE_IKKE_SØKT_SVP.getKode())
                .vilkårTyper(List.of(BrevGrunnlag.Behandlingsresultat.VilkårType.SVANGERSKAPSPENGERVILKÅR))
                .build())
            .build();
    ...
    void en_opphørt_Periode_med_en_arbeidsgiver_fra_tikjent_ytelse() {
        ...
        var uttakArbeidsforhold = List.of(opprettUttakArbeidsforhold(ArbeidsforholdIkkeOppfyltÅrsak.INGEN, ARBEIDSGIVER_1_REF,
            List.of(opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE1_FOM, PERIODE1_TOM),
                opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.AVSLÅTT, PeriodeIkkeOppfyltÅrsak.PERIODEN_ER_IKKE_FØR_FØDSEL, PERIODE2_FOM,
                    PERIODE2_TOM))));
    ...
    void en_opphørt_periode_med_2_arbeidsgivere_fra_uttak_innvilget() {
        ...
        var behandling = brevGrunnlag().uuid(UUID.randomUUID())
            .behandlingsresultat(behandlingsresultat().avslagsårsak(Avslagsårsak.SØKER_ER_IKKE_BOSATT.getKode())
                .vilkårTyper(List.of(BrevGrunnlag.Behandlingsresultat.VilkårType.SVANGERSKAPSPENGERVILKÅR,
                    BrevGrunnlag.Behandlingsresultat.VilkårType.MEDLEMSKAPSVILKÅRET))
                .build())
            .build();

        var uttakArbeidsforhold = List.of(opprettUttakArbeidsforhold(ArbeidsforholdIkkeOppfyltÅrsak.INGEN, ARBEIDSGIVER_1_REF,
                List.of(opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE1_FOM, PERIODE1_TOM),
                    opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE1_FOM, PERIODE1_TOM))),
            opprettUttakArbeidsforhold(ArbeidsforholdIkkeOppfyltÅrsak.INGEN, ARBEIDSGIVER_2_REF,
                List.of(opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE2_FOM, PERIODE2_TOM),
                    opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE2_FOM, PERIODE2_TOM))));
    ...
    void opphør_død_før_uten_tilkjent_ytelse_ingen_stønadsdatoer() {
        ...
        var uttakArbeidsforhold = List.of(opprettUttakArbeidsforhold(ArbeidsforholdIkkeOppfyltÅrsak.INGEN, ARBEIDSGIVER_1_REF,
                List.of(opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE1_FOM, PERIODE1_TOM),
                    opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.AVSLÅTT, PeriodeIkkeOppfyltÅrsak.BRUKER_ER_DØD, PERIODE1_FOM, PERIODE1_TOM))),
            opprettUttakArbeidsforhold(ArbeidsforholdIkkeOppfyltÅrsak.INGEN, ARBEIDSGIVER_2_REF,
                List.of(opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.INNVILGET, null, PERIODE2_FOM, PERIODE2_TOM),
                    opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType.AVSLÅTT, PeriodeIkkeOppfyltÅrsak.BRUKER_ER_DØD, PERIODE2_FOM,
                        PERIODE2_TOM))));
    ...
    private BrevGrunnlag.SvangerskapspengerUttak.UttakArbeidsforhold opprettUttakArbeidsforhold(ArbeidsforholdIkkeOppfyltÅrsak arbeidsforholdIkkeOppfyltÅrsak,
                                                                                                String arbeidsgiverRef,
                                                                                                List<BrevGrunnlag.SvangerskapspengerUttak.Periode> perioder) {
    ...
    private BrevGrunnlag.SvangerskapspengerUttak.Periode opprettUttaksperiode(BrevGrunnlag.PeriodeResultatType periodeResultatType,
