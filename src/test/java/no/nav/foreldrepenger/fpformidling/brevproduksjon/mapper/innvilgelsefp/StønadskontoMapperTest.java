...
class StønadskontoMapperTest {
    ...
    void skal_finne_disponible_dager_mødrekvote() {
        var stønadskontoer = List.of(stønadskonto().stønadskontotype(BrevGrunnlag.ForeldrepengerUttak.Stønadskonto.Type.MØDREKVOTE)
            .maxDager(10)
            .saldo(5)
            .kontoUtvidelser(kontoUtvidelser().build())
            .build());
        var dager = StønadskontoMapper.finnDisponibleDager(stønadskontoer, BrevGrunnlag.RelasjonsRolleType.MORA);
        assertThat(dager).isEqualTo(5);
    }

    @Test
    void skal_finne_disponible_dager_fellesperiode() {
        var stønadskontoer = List.of(stønadskonto().stønadskontotype(BrevGrunnlag.ForeldrepengerUttak.Stønadskonto.Type.FELLESPERIODE)
            .maxDager(10)
            .saldo(5)
            .kontoUtvidelser(kontoUtvidelser().build())
            .build());
    ...
    void skal_finne_disponible_dager_foreldrepenger() {
        var stønadskontoer = List.of(stønadskonto().stønadskontotype(BrevGrunnlag.ForeldrepengerUttak.Stønadskonto.Type.FORELDREPENGER)
            .maxDager(10)
            .saldo(5)
            .kontoUtvidelser(kontoUtvidelser().build())
            .build());
    ...
    void skal_finne_disponible_dager_fedrekvote() {
        var stønadskontoer = List.of(stønadskonto().stønadskontotype(BrevGrunnlag.ForeldrepengerUttak.Stønadskonto.Type.FEDREKVOTE)
            .maxDager(10)
            .saldo(5)
            .kontoUtvidelser(kontoUtvidelser().build())
            .build());
        var dager = StønadskontoMapper.finnDisponibleDager(stønadskontoer, BrevGrunnlag.RelasjonsRolleType.FARA);
        assertThat(dager).isEqualTo(5);
    }
