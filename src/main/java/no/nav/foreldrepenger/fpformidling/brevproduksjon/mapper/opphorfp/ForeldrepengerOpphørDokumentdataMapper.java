...
import static no.nav.foreldrepenger.fpformidling.integrasjon.fpsak.dto.behandling.BrevGrunnlag.FamilieHendelse;
import static no.nav.foreldrepenger.fpformidling.integrasjon.fpsak.dto.behandling.BrevGrunnlag.ForeldrepengerUttak;
import static no.nav.foreldrepenger.fpformidling.integrasjon.fpsak.dto.behandling.BrevGrunnlag.RelasjonsRolleType;
...
public class ForeldrepengerOpphørDokumentdataMapper implements DokumentdataMapper {
    ...
    public ForeldrepengerOpphørDokumentdata mapTilDokumentdata(DokumentFelles dokumentFelles,
        ...
        var uttaksperioder = Optional.ofNullable(behandling.foreldrepengerUttak())
            .map(ForeldrepengerUttak::perioderSøker)
            .orElse(List.of());
    ...
    private List<String> mapAvslagårsaker(Collection<VilkårType> vilkårTyper,
                                          ...
                                          List<ForeldrepengerUttak.Periode> perioder) {
    ...
    private Optional<LocalDate> finnStønadTomDatoHvisFinnes(List<ForeldrepengerUttak.Periode> uttaksperioder) {
        return uttaksperioder.stream()
            .filter(ForeldrepengerUttak.Periode::isInnvilget)
