...
public class PeriodeBeregner {
    ...
    private static final Map<TilkjentYtelseDagytelseDto.Aktivitetstatus, BrevGrunnlag.UttakArbeidType> uttakAktivitetStatusMap = new EnumMap<>(
        TilkjentYtelseDagytelseDto.Aktivitetstatus.class);

    static {
        uttakAktivitetStatusMap.put(TilkjentYtelseDagytelseDto.Aktivitetstatus.ARBEIDSTAKER, BrevGrunnlag.UttakArbeidType.ORDINÆRT_ARBEID);
        uttakAktivitetStatusMap.put(TilkjentYtelseDagytelseDto.Aktivitetstatus.FRILANSER, BrevGrunnlag.UttakArbeidType.FRILANS);
        uttakAktivitetStatusMap.put(TilkjentYtelseDagytelseDto.Aktivitetstatus.SELVSTENDIG_NÆRINGSDRIVENDE,
            BrevGrunnlag.UttakArbeidType.SELVSTENDIG_NÆRINGSDRIVENDE);
    }
    ...
    public static ForeldrepengerUttak.Periode finnUttaksperiode(TilkjentYtelsePeriodeDto tilkjentPeriode,
                                                                List<ForeldrepengerUttak.Periode> uttakPerioder) {
    ...
    public static int finnAntallTilkjentePerioderForUttaksperioden(List<TilkjentYtelsePeriodeDto> tilkjentPeriodeListe,
                                                                   ForeldrepengerUttak.Periode uttakPeriode) {
    ...
    public static boolean alleAktiviteterHarNullUtbetaling(List<ForeldrepengerUttak.Aktivitet> uttakAktiviteter) {
        return uttakAktiviteter.stream().allMatch(aktivitet -> aktivitet.utbetalingsgrad().compareTo(BigDecimal.ZERO) == 0);
    }

    public static Optional<ForeldrepengerUttak.Aktivitet> finnAktivitetMedStatusHvisFinnes(List<BrevGrunnlag.ForeldrepengerUttak.Aktivitet> uttakAktiviteter,
                                                                                           TilkjentYtelseDagytelseDto.TilkjentYtelseAndelDto andel) {
        ...
        for (var aktivitet : uttakAktiviteter) {
            if (uttakAktivitetStatusMap.getOrDefault(andel.aktivitetstatus(), BrevGrunnlag.UttakArbeidType.ANNET).equals(aktivitet.uttakArbeidType())
    ...
    public static Optional<ForeldrepengerUttak.Stønadskonto> finnStønadsKontoMedType(List<ForeldrepengerUttak.Stønadskonto> stønadskontoer,
                                                                                     ForeldrepengerUttak.Stønadskonto.Type stønadskontoType) {
