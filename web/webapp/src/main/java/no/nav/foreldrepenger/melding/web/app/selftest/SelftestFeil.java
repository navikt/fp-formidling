...
interface SelftestFeil extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "FPFORMIDLING-635121", feilmelding = "Klarte ikke å lese build time properties fil", logLevel = LogLevel.ERROR)
    Feil klarteIkkeÅLeseBuildTimePropertiesFil(IOException e);

    @TekniskFeil(feilkode = "FPFORMIDLING-287026", feilmelding = "Dupliserte selftest navn %s", logLevel = WARN)
    Feil dupliserteSelftestNavn(String name);

    @TekniskFeil(feilkode = "FPFORMIDLING-409676", feilmelding = "Uventet feil", logLevel = ERROR)
    Feil uventetSelftestFeil(IOException e);

    @TekniskFeil(feilkode = "FPFORMIDLING-932415", feilmelding = "Selftest ERROR: %s. Endpoint: %s. Responstid: %s. Feilmelding: %s.", logLevel = ERROR)
