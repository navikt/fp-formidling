...
class SvangerskapspengerInnvilgelseDokumentdataMapperTest {
    ...
    void skal_mappe_felter_for_førstegangsbehandling() {
        // Arrange
        var behandling = opprettBehandling(BrevGrunnlag.BehandlingType.FØRSTEGANGSSØKNAD).tilkjentYtelse(
                tilkjentYtelse().dagytelse(opprettTilkjentYtelse()).build())
            .svangerskapspengerUttak(opprettSvpUttaksresultat())
    ...
    private BrevGrunnlagBuilder opprettBehandling(BrevGrunnlag.BehandlingType behandlingType) {
        return brevGrunnlag().saksnummer(SAKSNUMMER)
            .behandlingType(behandlingType)
            .behandlingsresultat(
                behandlingsresultat().behandlingResultatType(BrevGrunnlag.Behandlingsresultat.BehandlingResultatType.INNVILGET).build())
    ...
    private BrevGrunnlag.SvangerskapspengerUttak opprettSvpUttaksresultat() {
        var periode1 = svangerskapspengerUttakPeriode().fom(PERIODE1_FOM)
            .tom(PERIODE1_TOM)
            .utbetalingsgrad(UTBETALINGSGRAD_PERIODE1)
        ...
        var periode2 = svangerskapspengerUttakPeriode().fom(PERIODE2_FOM)
            .tom(PERIODE2_TOM)
            .utbetalingsgrad(UTBETALINGSGRAD_PERIODE2)
            .periodeResultatType(BrevGrunnlag.PeriodeResultatType.INNVILGET)
            .build();
        ...
        var arbeidsforhold2 = svangerskapspengerUttakArbeidsforhold().arbeidType(BrevGrunnlag.UttakArbeidType.ORDINÆRT_ARBEID)
            .arbeidsgiverReferanse(ARBEIDSGIVER2_ORGNR)
            .perioder(List.of(periode3))
            .build();

        return svangerskapspengerUttak().uttakArbeidsforhold(List.of(arbeidsforhold1, arbeidsforhold2)).build();
    }
}
