...
public final class ForMyeUtbetaltMapper {
    private static final List<Årsak> manglendeEllerForSenSøknadOmGraderingÅrsaker = List.of(
        Årsak.of(ARBEIDER_I_UTTAKSPERIODEN_MER_ENN_0_PROSENT.getKode()), Årsak.of(AVSLAG_GRADERING_PÅ_GRUNN_AV_FOR_SEN_SØKNAD.getKode()),
        Årsak.of(FOR_SEN_SØKNAD.getKode()));

    private static final List<Årsak> innvilgetUtsettelsePgaFerieÅrsaker = List.of(Årsak.of(UTSETTELSE_GYLDIG_PGA_FERIE.getKode()),
        Årsak.of(UTSETTELSE_GYLDIG_PGA_FERIE_KUN_FAR_HAR_RETT.getKode()));

    private static final List<Årsak> innvilgetUtsettelsePgaArbeidÅrsaker = List.of(Årsak.of(UTSETTELSE_GYLDIG_PGA_100_PROSENT_ARBEID.getKode()),
        Årsak.of(UTSETTELSE_GYLDIG_PGA_ARBEID_KUN_FAR_HAR_RETT.getKode()));

    private ForMyeUtbetaltMapper() {
    }

    public static ForMyeUtbetalt forMyeUtbetalt(List<Vedtaksperiode> periodeListe, BrevGrunnlagDto behandling) {
        if (!behandling.erRevurdering()) {
            return null;
        }

        var vedtaksdato = behandling.avsluttet() != null ? behandling.avsluttet().toLocalDate() : null;
    ...
    private static ForMyeUtbetalt forMyeUtbetaltKode(boolean generell,
                                                     boolean ferie,
                                                     boolean jobb,
                                                     LocalDate innvilgetUtsettelseFOM,
