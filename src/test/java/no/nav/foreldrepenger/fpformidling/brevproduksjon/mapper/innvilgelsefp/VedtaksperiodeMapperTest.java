...
class VedtaksperiodeMapperTest {
    ...
    void skal_hente_dato_fra_uttaksperiode_når_denne_er_før_tilkjentytelseperioden_og_det_er_første_tilkjentYtelsePeriode() {
        ...
        var tyPeriode = TilkjentYtelsePeriode.ny().medPeriode(tidsperiodeBp1).medDagsats(0L).build();
        var tyPeriode2 = TilkjentYtelsePeriode.ny().medPeriode(tidsperiodeBp2).medDagsats(0L).build();
        var tyPeriode3 = TilkjentYtelsePeriode.ny().medPeriode(tidsperiodeBp3).medDagsats(620L).build();
        var tilkjentYtelsePerioder = of(tyPeriode, tyPeriode2, tyPeriode3);

        var bgPeriode = BeregningsgrunnlagPeriode.ny().medPeriode(beregningPer).medDagsats(620L).build();
        var beregningsgrunnlagPerioder = of(bgPeriode);

        var uttakAktivitet = UttakResultatPeriodeAktivitet.ny().medTrekkdager(BigDecimal.TEN).medUtbetalingsprosent(BigDecimal.ZERO).build();
    ...
    private ForeldrepengerUttak uttak(List<UttakResultatPeriode> søkersPerioder, List<UttakResultatPeriode> annenPartPerioder) {
        return new ForeldrepengerUttak(søkersPerioder, annenPartPerioder);
    }
    ...
    void skal_sortere_perioder_uten_beregningsgrunnlag_også() {
        ...
        var tyPeriode1 = TilkjentYtelsePeriode.ny().medPeriode(tidsperiodeBp1).medDagsats(620L).build();
        ...
        var uPeriode1 = UttakResultatPeriode.ny()
            ...
            .medPeriodeResultatType(PeriodeResultatType.INNVILGET)
            .build();
        var uttakAktivitet2 = UttakResultatPeriodeAktivitet.ny().medTrekkdager(BigDecimal.TEN).medUtbetalingsprosent(BigDecimal.ZERO).build();
        var uPeriode2 = UttakResultatPeriode.ny()
            .medAktiviteter(of(uttakAktivitet2))
            .medTidsperiode(tidsperiodeUp2)
    ...
    void skal_mappe_annenAktitetListe_når_den_inneholder_annet() {
        ...
        var tilkjentYtelseAndel = TilkjentYtelseAndel.ny().medAktivitetStatus(AktivitetStatus.KOMBINERT_AT_FL).build();
        var tyPeriode = TilkjentYtelsePeriode.ny().medPeriode(tidsperiodeBp1).medDagsats(0L).medAndeler(of(tilkjentYtelseAndel)).build();
        ...
        var uttakAktivitet = UttakAktivitet.ny().medUttakArbeidType(UttakArbeidType.ANNET).build();
    ...
    void skal_ignorere_avslåtte_manglende_søkte_perioder_med_null_trekkdager_ved_mapping_av_periodeliste() {
        ...
        var tyPeriode = TilkjentYtelsePeriode.ny().medPeriode(tidsperiode).build();
        var tilkjentYtelsePerioder = of(tyPeriode);
        var uttakAktivitet = UttakResultatPeriodeAktivitet.ny().medTrekkdager(BigDecimal.ZERO).medUtbetalingsprosent(BigDecimal.ZERO).build();
        var uPeriode = UttakResultatPeriode.ny()
            .medAktiviteter(of(uttakAktivitet))
