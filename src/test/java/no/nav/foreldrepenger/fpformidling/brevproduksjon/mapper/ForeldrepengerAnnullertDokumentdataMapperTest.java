...
class ForeldrepengerAnnullertDokumentdataMapperTest {
    ...
    void before() {
        ...
        dokumentdataMapper = new ForeldrepengerAnnullertDokumentdataMapper(brevParametere);
    }

    @Test
    void skal_mappe_felter_for_brev_til_bruker_med_ny_startdato() {
        // Arrange
        var behandling = defaultBuilder().fagsakYtelseType(BrevGrunnlagDto.FagsakYtelseType.FORELDREPENGER)
            .foreldrepenger(BrevGrunnlagBuilders.foreldrepenger().nyStartDatoVedUtsattOppstart(NY_STARTDATO).build())
            .build();
